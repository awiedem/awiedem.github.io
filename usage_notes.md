...
| **SH 1983** | Participation remains incomplete and inconsistent: 135 rows lack participation fields, and 253 have more valid votes than voters. Avoid reconstructing statewide turnout or vote counts; party shares have not been independently validated. |
| **Legacy postal flag** | `flag_briefwahl_only` tests zero electorate with positive votes before cleanup; it also catches missing/corrupt fields. It does not establish postal-district status. The [source limitation table](https://github.com/awiedem/german_election_data/blob/main/data/state_elections/metadata/source_limitations.csv) documents the remaining SH 1983 limitation. |
